    ...
    public partial class GameDetails : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            switch (Globals.selectedGame)
            {
                ...
                case "hl2": { GameIndexRow = 1; pointer = 0; offset = 1; break; }
        ...
        private void SendInputToArduino()
        {
            if (Globals.isConnected == true)
            {
                ...
                try
                {
                    if (valueKM < 2)
                    {
                        Globals.port.Write("#LEDA\n");
                    }
                    else if (valueKM >= 2 && valueKM < 40)
                    {
                        Globals.port.Write("#LED0\n");
                        //Globals.port.Write("#LED1\n");
                    }
                    else if (valueKM >= 40 && valueKM < 80)
                    {
                        Globals.port.Write("#LED1\n");

                        //Globals.port.Write("#LED2\n");
                    }
                    else if (valueKM >= 80 && valueKM < 120)
                    {
                        Globals.port.Write("#LED2\n");

                       // Globals.port.Write("#LED3\n");
                    }
                    else if (valueKM >= 120 && valueKM < 160)
                    {
                        Globals.port.Write("#LED3\n");

                        //Globals.port.Write("#LED4\n");
                    }
                    else if (valueKM >= 160 && valueKM < 200)
                    {
                        Globals.port.Write("#LED4\n");

                        //Globals.port.Write("#LED5\n");
                    }
                    else if (valueKM >= 200 && valueKM < 240)
                    {
                        Globals.port.Write("#LED5\n");

                        //Globals.port.Write("#LED6\n");
                    }
                    else if (valueKM >= 240 && valueKM < 280)
                    {
                        Globals.port.Write("#LED6\n");

                       // Globals.port.Write("#LED7OF\n");
                    }
                    else if (valueKM >= 280 && valueKM < 320)
                    {
                        Globals.port.Write("#LED7\n");

                        //Globals.port.Write("#LED8OF\n");
                    }
                    else if (valueKM >= 320 && valueKM < 360)
                    {
                        Globals.port.Write("#LED8\n");

                       // Globals.port.Write("#LED9OF\n");
                    }
                    else if (valueKM >= 360 && valueKM < 400)
                    {
                        Globals.port.Write("#LED9\n");
